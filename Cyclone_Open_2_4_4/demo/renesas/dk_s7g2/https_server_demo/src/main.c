...
#define APP_IF1_MAC_ADDR "00-AB-CD-00-72-01"
...
#define APP_IF2_MAC_ADDR "00-AB-CD-00-72-02"
...
void ioInit(void)
{
   ...
   //Configure LED4
   R_PFS->LED4_PFS_b.PMR = 0;
   R_PFS->LED4_PFS_b.PDR = 1;

   //Configure LED5
   R_PFS->LED5_PFS_b.PMR = 0;
   R_PFS->LED5_PFS_b.PDR = 1;

   //Configure S2
   R_PFS->S2_PFS_b.PMR = 0;
   R_PFS->S2_PFS_b.PDR = 0;

   //Configure S3
   R_PFS->S3_PFS_b.PMR = 0;
   R_PFS->S3_PFS_b.PDR = 0;

   //Configure S4
   R_PFS->S4_PFS_b.PMR = 0;
   R_PFS->S4_PFS_b.PDR = 0;

   //Configure ETH0 reset pin
   R_PFS->ETH0_RST_PFS_b.PMR = 0;
   R_PFS->ETH0_RST_PFS_b.PDR = 1;

   //Configure ETH1 reset pin
   R_PFS->ETH1_RST_PFS_b.PMR = 0;
   R_PFS->ETH1_RST_PFS_b.PDR = 1;
   ...
   LED4_IOPORT->PCNTR3 = LED4_MASK << 16;
   LED5_IOPORT->PCNTR3 = LED5_MASK << 16;

   //Reset the first PHY transceiver
   ETH0_RST_IOPORT->PCNTR3 = ETH0_RST_MASK << 16;
   sleep(10);
   ETH0_RST_IOPORT->PCNTR3 = ETH0_RST_MASK;
   sleep(10);

   //Reset the second PHY transceiver
   ETH1_RST_IOPORT->PCNTR3 = ETH1_RST_MASK << 16;
   sleep(10);
   ETH1_RST_IOPORT->PCNTR3 = ETH1_RST_MASK;
   sleep(10);
}
...
error_t eth1InterfaceInit(void)
{
   ...
   netSetDriver(interface, &s7g2Eth1Driver);
...
error_t eth2InterfaceInit(void)
{
   ...
   netSetMacAddr(interface, &macAddr);
   //Select the relevant MAC driver
   netSetDriver(interface, &s7g2Eth2Driver);
   //Select the relevant PHY driver
   netSetPhyDriver(interface, &ksz8091PhyDriver);
   //Set PHY address
   netSetPhyAddr(interface, 1);
   ...
   const char_t *param)
{
   ...
   {
      strcpy(connection->buffer, "DK-S7G2");
   }
