...
void ioInit(void)
{
   ...
   R_PFS->PHY_RST_PFS = R_PFS_PORT_PIN_PmnPFS_PDR_Msk;
   ...
   PHY_RST_PORT->PCNTR3 = PHY_RST_MASK << R_PORT0_PCNTR3_PORR_Pos;
   sleep(10);

   //Take the PHY transceiver out of reset
   PHY_RST_PORT->PCNTR3 = PHY_RST_MASK << R_PORT0_PCNTR3_POSR_Pos;
   sleep(10);
}
   ...
   bool_t *state)
{
   ...
   {
      //Retrieve the state of the S1 button
      *state = !(S1_PORT->PCNTR2 & S1_MASK);
   }
   else if(address == 10001)
   {
      //Retrieve the state of the S2 button
      *state = !(S2_PORT->PCNTR2 & S2_MASK);
   }
   ...
   bool_t state, bool_t commit)
{
   ...
   if(address == 0)
   {
      ...
      if(commit)
      {
         ...
         if(state)
            LED2_PORT->PCNTR3 = LED2_MASK << R_PORT0_PCNTR3_POSR_Pos;
         else
            LED2_PORT->PCNTR3 = LED2_MASK << R_PORT0_PCNTR3_PORR_Pos;
   ...
   else if(address == 1)
   {
      ...
      if(commit)
      {
         //Update the state of the LED
         if(state)
            LED3_PORT->PCNTR3 = LED3_MASK << R_PORT0_PCNTR3_POSR_Pos;
         else
            LED3_PORT->PCNTR3 = LED3_MASK << R_PORT0_PCNTR3_PORR_Pos;
   ...
   uint16_t *value)
{
   ...
   {
      //Retrieve the state of the S1 button
      *value = !(S1_PORT->PCNTR2 & S1_MASK);
   }
   else if(address == 30001)
   {
      //Retrieve the state of the S2 button
      *value = !(S2_PORT->PCNTR2 & S2_MASK);
   }
