...
error_t extIntInit(void)
{
   ...
   R_ICU->IRQCR_b[7].FCLKSEL = 0;
   //Enable digital filter
   R_ICU->IRQCR_b[7].FLTEN = 1;
   //Configure IRQ7 polarity (falling edge)
   R_ICU->IRQCR_b[7].IRQMD = 0;

   //Redirect the event to the NVIC
   R_ICU->IELSR_b[ICU_IRQ7_IRQn].DTCE = 0;

   //Configure IRQ7 interrupt priority
   NVIC_SetPriority(ICU_IRQ7_IRQn, NVIC_EncodePriority(3, 15, 0));
...
void ICU_IRQ7_IRQHandler(void)
{
   ...
   R_ICU->IELSR_b[ICU_IRQ7_IRQn].IR = 0;
