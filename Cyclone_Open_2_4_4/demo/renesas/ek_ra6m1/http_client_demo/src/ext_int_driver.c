...
error_t extIntInit(void)
{
   ...
   R_ICU->IRQCR_b[9].FLTEN = 0;

   //Unlock PFS registers
   R_PMISC->PWPR &= ~R_PMISC_PWPR_B0WI_Msk;
   R_PMISC->PWPR |= R_PMISC_PWPR_PFSWE_Msk;

   //Configure IRQ9 pin as an input
   R_PFS->PORT[3].PIN[4].PmnPFS = R_PFS_PORT_PIN_PmnPFS_ISEL_Msk;
   ...
   R_ICU->IRQCR_b[9].FCLKSEL = 0;
   //Enable digital filter
   R_ICU->IRQCR_b[9].FLTEN = 1;
   //Configure IRQ9 polarity (falling edge)
   R_ICU->IRQCR_b[9].IRQMD = 0;

   //Redirect the event to the NVIC
   R_ICU->IELSR_b[ICU_IRQ9_IRQn].DTCE = 0;

   //Configure IRQ9 interrupt priority
   NVIC_SetPriority(ICU_IRQ9_IRQn, NVIC_EncodePriority(3, 15, 0));
...
void ICU_IRQ9_IRQHandler(void)
{
   ...
   R_ICU->IELSR_b[ICU_IRQ9_IRQn].IR = 0;
