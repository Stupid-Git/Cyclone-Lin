...
error_t mp3DecoderStart(IcecastClientContext *icecastClientContext)
{
   ...
   taskParams.stackSize = MP3_DECODER_STACK_SIZE;
   taskParams.priority = MP3_DECODER_PRIORITY;

   //Create a task to decode the MP3 stream
   taskId = osCreateTask("MP3 Decoder", mp3DecoderTask, icecastClientContext,
      &taskParams);
