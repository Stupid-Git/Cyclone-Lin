...
void lcdPutChar(char_t c)
{
   ...
   else if(lcdLine < 16 && lcdColumn < 21)
   {
      ...
      R_LCD_DisplaySetCursor(lcdColumn, lcdLine);
      R_LCD_DisplayStr(buffer);
...
int_t main(void)
{
   ...
   netSetDriver(interface, &rx65nEthDriver);
   ...
   const char_t *param)
{
   ...
   {
      strcpy(connection->buffer, "RSK-RX65N");
   }
...
void vApplicationSetupTimerInterrupt(void)
{
   ...
   CMT1.CMCOR = ((configPERIPHERAL_CLOCK_HZ / configTICK_RATE_HZ) - 1) / 8;
