/**
 * @file system_ra6e1.c
 * @brief System configuration for Renesas RA6E1 device
 *
 * @section License
 *
...
void SystemInit(void)
{
   ...
   //Start HOCO clock operation
   R_SYSTEM->HOCOCR &= ~R_SYSTEM_HOCOCR_HCSTP_Msk;

   //Wait for the HOCO clock to stabilize
   while((R_SYSTEM->OSCSF & R_SYSTEM_OSCSF_HOCOSF_Msk) == 0)
   {
   }

   //Configure PLL source (HOCO clock), PLL multiplication ratio (x20) and
   //PLL input division ratio (/2)
   R_SYSTEM->PLLCCR = (1 << R_SYSTEM_PLLCCR_PLSRCSEL_Pos) |
      (39 << R_SYSTEM_PLLCCR_PLLMUL_Pos) | (1 << R_SYSTEM_PLLCCR_PLIDIV_Pos);
   ...
   R_SYSTEM->SCKDIVCR = (2 << R_SYSTEM_SCKDIVCR_FCK_Pos) |
      (0 << R_SYSTEM_SCKDIVCR_ICK_Pos) | (2 << R_SYSTEM_SCKDIVCR_BCK_Pos) |
