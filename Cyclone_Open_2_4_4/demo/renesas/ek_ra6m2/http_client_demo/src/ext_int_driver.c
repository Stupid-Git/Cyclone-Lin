...
error_t extIntInit(void)
{
   ...
   R_ICU->IRQCR_b[1].FLTEN = 0;

   //Unlock PFS registers
   R_PMISC->PWPR &= ~R_PMISC_PWPR_B0WI_Msk;
   R_PMISC->PWPR |= R_PMISC_PWPR_PFSWE_Msk;

   //Configure IRQ1 pin as an input
   R_PFS->PORT[1].PIN[4].PmnPFS = R_PFS_PORT_PIN_PmnPFS_ISEL_Msk;
   ...
   R_ICU->IRQCR_b[1].FCLKSEL = 0;
   //Enable digital filter
   R_ICU->IRQCR_b[1].FLTEN = 1;
   //Configure IRQ1 polarity (falling edge)
   R_ICU->IRQCR_b[1].IRQMD = 0;

   //Redirect the event to the NVIC
   R_ICU->IELSR_b[ICU_IRQ1_IRQn].DTCE = 0;

   //Configure IRQ1 interrupt priority
   NVIC_SetPriority(ICU_IRQ1_IRQn, NVIC_EncodePriority(3, 15, 0));
...
void ICU_IRQ1_IRQHandler(void)
{
   ...
   R_ICU->IELSR_b[ICU_IRQ1_IRQn].IR = 0;
