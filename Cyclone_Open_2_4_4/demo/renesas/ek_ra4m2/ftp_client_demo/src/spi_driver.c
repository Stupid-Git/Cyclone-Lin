...
error_t spiInit(void)
{
   ...
   R_MSTP->MSTPCRB &= ~(R_MSTP_MSTPCRB_MSTPB_Msk << 27);
   //Enable protection
   ...
   R_PFS->PORT[4].PIN[1].PmnPFS = R_PFS_PORT_PIN_PmnPFS_PDR_Msk;
   R_PORT4->PCNTR3 = (1 << 1) << R_PORT0_PCNTR3_POSR_Pos;

   //Configure SCK pin
   R_PFS->PORT[4].PIN[0].PmnPFS = R_PFS_PORT_PIN_PmnPFS_PMR_Msk |
      (4 << R_PFS_PORT_PIN_PmnPFS_PSEL_Pos);

   //Configure MOSI pin
   R_PFS->PORT[2].PIN[7].PmnPFS = R_PFS_PORT_PIN_PmnPFS_PMR_Msk |
      (4 << R_PFS_PORT_PIN_PmnPFS_PSEL_Pos);

   //Configure MISO pin
   R_PFS->PORT[2].PIN[6].PmnPFS = R_PFS_PORT_PIN_PmnPFS_PMR_Msk |
      (4 << R_PFS_PORT_PIN_PmnPFS_PSEL_Pos);
   ...
   R_SCI4->SMR = R_SCI0_SMR_CM_Msk;
   //Select master mode
   R_SCI4->SPMR = R_SCI0_SPMR_CKPH_Msk | R_SCI0_SPMR_MSS_Msk;
   //Set data transfer direction (MSB first)
   R_SCI4->SCMR |= R_SCI0_SCMR_SDIR_Msk;
   //Set bit rate
   R_SCI4->BRR = 2;

   //Enable SCI module
   R_SCI4->SCR = R_SCI0_SCR_TE_Msk | R_SCI0_SCR_RE_Msk;
...
{
   //Assert CS signal
   R_PORT4->PCNTR3 = (1 << 1) << R_PORT0_PCNTR3_PORR_Pos;
   //CS setup time
   usleep(1);
}
...
uint8_t spiTransfer(uint8_t data)
{
   ...
   while((R_SCI4->SSR & R_SCI0_SSR_TEND_Msk) == 0)
   {
   }
