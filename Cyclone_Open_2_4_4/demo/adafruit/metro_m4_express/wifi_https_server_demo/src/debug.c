...
void debugInit(uint32_t baudrate)
{
   ...
   //Enable SERCOM3 core clock
   GCLK_REGS->GCLK_PCHCTRL[SERCOM3_GCLK_ID_CORE] = GCLK_PCHCTRL_GEN_GCLK0 |
      GCLK_PCHCTRL_CHEN_Msk;

   //Enable PORT bus clock (CLK_PORT_APB)
   MCLK_REGS->MCLK_APBBMASK |= MCLK_APBBMASK_PORT_Msk;
   //Enable SERCOM3 bus clock (CLK_SERCOM3_APB)
   MCLK_REGS->MCLK_APBBMASK |= MCLK_APBBMASK_SERCOM3_Msk;

   //Configure RXD pin (PA23)
   PORT_REGS->GROUP[0].PORT_PINCFG[23] |= PORT_PINCFG_PULLEN_Msk;
   PORT_REGS->GROUP[0].PORT_PINCFG[23] |= PORT_PINCFG_PMUXEN_Msk;
   temp = PORT_REGS->GROUP[0].PORT_PMUX[11] & ~PORT_PMUX_PMUXO_Msk;
   PORT_REGS->GROUP[0].PORT_PMUX[11] = temp | PORT_PMUX_PMUXO(MUX_PA23C_SERCOM3_PAD1);

   //Configure TXD pin (PA22)
   PORT_REGS->GROUP[0].PORT_PINCFG[22] |= PORT_PINCFG_PMUXEN_Msk;
   temp = PORT_REGS->GROUP[0].PORT_PMUX[11] & ~PORT_PMUX_PMUXE_Msk;
   PORT_REGS->GROUP[0].PORT_PMUX[11] = temp | PORT_PMUX_PMUXE(MUX_PA22C_SERCOM3_PAD0);
   ...
   //Configure baud rate
   SERCOM3_REGS->USART_INT.SERCOM_BAUD = 65535 - ((baudrate * 16384) / (SystemCoreClock / 64));
