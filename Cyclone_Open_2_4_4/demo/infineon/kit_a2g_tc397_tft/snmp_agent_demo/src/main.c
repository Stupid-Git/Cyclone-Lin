...
#define APP_MAC_ADDR "00-AB-CD-EF-03-97"
...
#define APP_IPV6_LINK_LOCAL_ADDR "fe80::397"
#define APP_IPV6_PREFIX "2001:db8::"
#define APP_IPV6_PREFIX_LENGTH 64
#define APP_IPV6_GLOBAL_ADDR "2001:db8::397"
 ...
 * @param reloadValue Timer reload value
 **/

void timerIrqHandler(int_t reloadValue)
{
   uint_t cpuId;
   Ifx_STM *stm;

   //Get CPU core identifier
   cpuId = _mfcr(CPU_CORE_ID) & IFX_CPU_CORE_ID_CORE_ID_MSK;

   //Select the relevant STM module
   if(cpuId == 0)
   {
      stm = &MODULE_STM0;
   }
   ...
   stm->CMP[0].U += (uint32_t) reloadValue;
...
void timerInit(void)
{
   ...
   {
      stm = &MODULE_STM0;
      srcId = SRC_ID_STM0SR0;
   }
   else if(cpuId == 1)
   {
      stm = &MODULE_STM1;
      srcId = SRC_ID_STM1SR0;
   }
   else if(cpuId == 2)
   {
      stm = &MODULE_STM2;
      srcId = SRC_ID_STM2SR0;
   }
   else if(cpuId == 3)
   {
      stm = &MODULE_STM3;
      srcId = SRC_ID_STM3SR0;
   }
   else if(cpuId == 4)
   {
      stm = &MODULE_STM4;
      srcId = SRC_ID_STM4SR0;
   }
   else
   {
      stm = &MODULE_STM5;
      srcId = SRC_ID_STM5SR0;
   }

   //Get STM clock frequency
   reloadValue = SYSTEM_GetStmClock() / 1000;

   //Set initial compare value
   stm->CMP[0].U = stm->TIM0.U + reloadValue;
   ...
   InterruptInstall(srcId, timerIrqHandler, 2, (int_t) reloadValue);
   //Enable interrupts
   InterruptEnable(srcId);
}
...
int_t main(void)
{
   ...
   netSetDriver(interface, &tc3xxEthDriver);
