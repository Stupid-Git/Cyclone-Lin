   ...
   bool_t *state)
{
   ...
   else if(address >= 3 && address <= 9)
   {
      //Coils 3 to 9 always read as zero
      *state = 0;
   }
   else if(address == 10000)
   {
      //Retrieve the state of the BTN0 button
      *state = !GPIO_PinInGet(BTN0_GPIO_PORT, BTN0_GPIO_PIN);
   }
   else if(address == 10001)
   {
      //Retrieve the state of the BTN1 button
   ...
   bool_t state, bool_t commit)
{
   ...
   if(address == 0)
   {
      ...
      if(commit)
      {
         ...
         if(state)
            GPIO_PinOutClear(LED0_R_GPIO_PORT, LED0_R_GPIO_PIN);
         else
            GPIO_PinOutSet(LED0_R_GPIO_PORT, LED0_R_GPIO_PIN);
   ...
   else if(address == 1)
   {
      ...
      if(commit)
      {
         //Update the state of the LED
         if(state)
            GPIO_PinOutClear(LED0_G_GPIO_PORT, LED0_G_GPIO_PIN);
         else
            GPIO_PinOutSet(LED0_G_GPIO_PORT, LED0_G_GPIO_PIN);
   ...
   else if(address == 2)
   {
      ...
      if(commit)
      {
         //Update the state of the LED
         if(state)
            GPIO_PinOutClear(LED0_B_GPIO_PORT, LED0_B_GPIO_PIN);
         else
            GPIO_PinOutSet(LED0_B_GPIO_PORT, LED0_B_GPIO_PIN);
   ...
   {
      //Writes to coils 3 to 9 are ignored
   }
   else
   {
      //The register address is not acceptable
   ...
   uint16_t *value)
{
   ...
   {
      //Retrieve the state of the BTN0 button
      *value = !GPIO_PinInGet(BTN0_GPIO_PORT, BTN0_GPIO_PIN);
   }
   else if(address == 30001)
   {
      //Retrieve the state of the BTN1 button
      *value = !GPIO_PinInGet(BTN1_GPIO_PORT, BTN1_GPIO_PIN);
   }
   ...
   else if(address >= 40003 && address <= 40009)
   {
      //Registers 40003 to 40009 always read as zero
      *value = 0;
   }
   else
   {
   ...
   uint16_t value, bool_t commit)
{
   ...
   {
      //Writes to registers 40003 to 40009 are ignored
   }
   else
   {
      //The register address is not acceptable
