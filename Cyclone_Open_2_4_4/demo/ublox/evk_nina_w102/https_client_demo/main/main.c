...
#define APP_IF2_HOST_NAME "https-client-demo"
...
void userTask(void *param)
{
   ...
   {
      //SW1 button pressed?
      if(!gpio_get_level(33))
      {
         //Check wether the Wi-Fi interface is up
         if(netGetLinkState(&netInterface[1]))
         {
            //HTTP client test routine (Wi-Fi interface)
            httpClientTest(&netInterface[1]);
         }
         else
         {
            //HTTP client test routine (Ethernet interface)
