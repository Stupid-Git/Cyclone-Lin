 ...
 * \file
 *
 * \brief System configuration file for SAME53
 *
...
void SystemInit(void)
{
    ...
    // Use external clock (25 MHz)
    OSCCTRL_REGS->OSCCTRL_XOSCCTRL[1] = OSCCTRL_XOSCCTRL_XTALEN(0) |
    ...
    // Set up DPLL0 to output 120MHz using external clock divided by 16 (1.25 MHz)
    OSCCTRL_REGS->DPLL[0].OSCCTRL_DPLLRATIO = OSCCTRL_DPLLRATIO_LDRFRAC(0) |
        OSCCTRL_DPLLRATIO_LDR(95);

    OSCCTRL_REGS->DPLL[0].OSCCTRL_DPLLCTRLB = OSCCTRL_DPLLCTRLB_DIV(9) |
