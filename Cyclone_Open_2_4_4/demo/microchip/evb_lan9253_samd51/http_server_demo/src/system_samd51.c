 ...
 * \file
 *
 * \brief System configuration file for SAMD51
 *
...
void SystemInit(void)
{
    ...
    // Run with a 16MHz external crystal on XOSC0
    OSCCTRL_REGS->OSCCTRL_XOSCCTRL[0] = OSCCTRL_XOSCCTRL_ENALC(1) |
    ...
    while((OSCCTRL_REGS->OSCCTRL_STATUS & OSCCTRL_STATUS_XOSCRDY0_Msk) == 0)
    {
    }

    // Set up DPLL0 to output 120MHz using XOSC0 output divided by 16
    OSCCTRL_REGS->DPLL[0].OSCCTRL_DPLLRATIO = OSCCTRL_DPLLRATIO_LDRFRAC(0) |
        OSCCTRL_DPLLRATIO_LDR(119);

    OSCCTRL_REGS->DPLL[0].OSCCTRL_DPLLCTRLB = OSCCTRL_DPLLCTRLB_DIV(7) |
        OSCCTRL_DPLLCTRLB_REFCLK(2);
