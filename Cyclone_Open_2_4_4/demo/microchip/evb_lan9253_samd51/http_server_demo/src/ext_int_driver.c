...
error_t extIntInit(void)
{
   ...
   //Configure IRQ pin (PA19)
   PORT_REGS->GROUP[0].PORT_DIRCLR = PORT_PA19;
   PORT_REGS->GROUP[0].PORT_OUTSET = PORT_PA19;
   ...
   //Select peripheral function  (EXTINT3)
   temp = PORT_REGS->GROUP[0].PORT_PMUX[9] & ~PORT_PMUX_PMUXO_Msk;
   PORT_REGS->GROUP[0].PORT_PMUX[9] = temp | PORT_PMUX_PMUXO(MUX_PA19A_EIC_EXTINT3);

   //Configure the EIC input sense and filtering
   EIC_REGS->EIC_CONFIG[0] = (EIC_REGS->EIC_CONFIG[0] & ~EIC_CONFIG_SENSE3_Msk) |
   ...
   EIC_REGS->EIC_INTENSET = EIC_INTENSET_EXTINT(1 << 3);

   //Set priority grouping
   NVIC_SetPriorityGrouping(4);

   //Configure EIC interrupt priority
   NVIC_SetPriority(EIC_EXTINT_3_IRQn, NVIC_EncodePriority(4, 7, 0));

   //Set EIC interrupt to pending
   NVIC_SetPendingIRQ(EIC_EXTINT_3_IRQn);
...
void EIC_EXTINT_3_Handler(void)
{
   ...
   EIC_REGS->EIC_INTFLAG = EIC_INTFLAG_EXTINT(1 << 3);

   //Ensure the IRQ pin is asserted
   if(!(PORT_REGS->GROUP[0].PORT_IN & PORT_PA19))
   {
