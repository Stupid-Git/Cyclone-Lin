...
void debugInit(uint32_t baudrate)
{
   ...
   //Configure RXD pin (PB13)
   PORT_REGS->GROUP[1].PORT_PINCFG[13] |= PORT_PINCFG_PULLEN_Msk;
   ...
   while((SERCOM4_REGS->USART_INT.SERCOM_SYNCBUSY & SERCOM_USART_INT_SYNCBUSY_SWRST_Msk) != 0)
   {
   }

   //Configure SERCOM4
   SERCOM4_REGS->USART_INT.SERCOM_CTRLA = SERCOM_USART_INT_CTRLA_DORD_Msk |
   ...
   //Configure baud rate
   SERCOM4_REGS->USART_INT.SERCOM_BAUD = 65535 - ((baudrate * 16384) / (SystemCoreClock / 64));
...
int_t fputc(int_t c, FILE *stream)
{
   ...
   {
      //Send character
      SERCOM4_REGS->USART_INT.SERCOM_DATA = c;

      //Wait for the transfer to complete
      while((SERCOM4_REGS->USART_INT.SERCOM_INTFLAG & SERCOM_USART_INT_INTFLAG_TXC_Msk) == 0)
      {
      }
