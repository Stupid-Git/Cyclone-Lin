...
#include "pic32mm_usb_curiosity.h"
#include "core/net.h"
...
/**
 * @brief External 4 interrupt service routine
 **/

void __ISR(_EXTERNAL_4_VECTOR, IPL2SOFT) ext4IrqHandler(void)
{
...
void systemInit(void)
{
   ...
   //Temporarily switch to 8MHz FRC (without PLL)
   OSCCON &= ~(_OSCCON_FRCDIV_MASK | _OSCCON_NOSC_MASK | _OSCCON_CLKLOCK_MASK |
      _OSCCON_OSWEN_MASK);

   //Perform clock switching if necessary
   if(OSCCONbits.COSC != OSCCONbits.NOSC)
   {
      //Initiate clock switching operation
      OSCCONbits.OSWEN = 1;

      //Wait for clock switching to complete
      while(OSCCONbits.OSWEN != 0)
      {
      }
   }

   //Configure PLL (PLLODIV = /4, PLLMULT = 12x, PLL source = FRC)
   SPLLCON = (2 << _SPLLCON_PLLODIV_POSITION) |
      (5 << _SPLLCON_PLLMULT_POSITION) | _SPLLCON_PLLICLK_MASK;

   //Switch to the PLL source
   OSCCON |= (1 << _OSCCON_NOSC_POSITION) | _OSCCON_OSWEN_MASK;

   //Wait for PLL to lock and clock switching operation to complete
   while(CLKSTATbits.SPLLRDY == 0 || OSCCONbits.OSWEN != 0)
   {
   }

   //Enable FRC self-tuning
   OSCTUN = _OSCTUN_ON_MASK | _OSCTUN_SRC_MASK;
...
void ioInit(void)
{
   //Configure LED1 (RD3) and LED3_R (RD1)
   TRISDCLR = LED1_MASK | LED3_R_MASK;
   //Clear LEDs
   LATDCLR = LED1_MASK | LED3_R_MASK;
   //Disable analog pads
   ANSELDCLR = LED1_MASK | LED3_R_MASK;

   //Configure LED2 (RC13), LED3_G (RC3) and LED3_B (RC15)
   TRISCCLR = LED2_MASK | LED3_G_MASK | LED3_B_MASK;
   //Clear LEDs
   LATCCLR = LED2_MASK | LED3_G_MASK | LED3_B_MASK;
   //Disable analog pads
   ANSELCCLR = LED2_MASK | LED3_G_MASK | LED3_B_MASK;
   ...
   //Configure SW2 (RC10) and SW3 (RC4))
   TRISCSET = SW2_MASK | SW3_MASK;
   //Enable pull-up
   CNPUCSET = SW2_MASK | SW3_MASK;
   //Disable analog pads
   ANSELCCLR = SW2_MASK | SW3_MASK;

   //Configure ENC28J60 CS (RB2)
...
void timer1Init(void)
{
   ...
   //Set period register
   PR1 = ((24000000 / 8) / 1000) - 1;
   //Clear timer register
   TMR1 = 0;

   //Set interrupt priority
   IPC4CLR = _IPC4_T1IP_MASK;
   IPC4SET = (4 << _IPC4_T1IP_POSITION);
   //Set interrupt subpriority
   IPC4CLR = _IPC4_T1IS_MASK;
   IPC4SET = (0 << _IPC4_T1IS_POSITION);
...
int_t main(void)
{
   ...
   TRACE_INFO("Target: PIC32MM0256GPM064\r\n");
   TRACE_INFO("\r\n");
