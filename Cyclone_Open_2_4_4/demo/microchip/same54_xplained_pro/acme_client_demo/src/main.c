...
char_t certPublicKey[512];
char_t certPrivateKey[2048];
...
error_t acmeTestGenerateCertKeys(void)
{
   ...
   do
   {
      ...
      error = rsaGenerateKeyPair(YARROW_PRNG_ALGO, &yarrowContext, 2048, 65537,
         &privateKey, &publicKey);
      //Any error to report?
      if(error)
         break;
      ...
      error = pemExportRsaPrivateKey(&privateKey, certPrivateKey, &n);
      //Any error to report?
      if(error)
         break;
      ...
      error = pemExportRsaPublicKey(&publicKey, certPublicKey, &n);
      //Any error to report?
      if(error)
         break;
