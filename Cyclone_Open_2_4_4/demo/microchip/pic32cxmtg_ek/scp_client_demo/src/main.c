...
void ioInit(void)
{
   ...
   PMC_REGS->PMC_PCR = PMC_PCR_PID(ID_PIOD);
   temp = PMC_REGS->PMC_PCR;
   PMC_REGS->PMC_PCR = temp | PMC_PCR_CMD_Msk | PMC_PCR_EN_Msk;

   //Configure green LED
   PIO_GROUP_LED_G.PIO_MSKR = LED_G;
   PIO_GROUP_LED_G.PIO_CFGR = PIO_CFGR_DIR_OUTPUT | PIO_CFGR_FUNC_GPIO;
   PIO_GROUP_LED_G.PIO_SODR = LED_G;

   //Configure blue LED
   PIO_GROUP_LED_B.PIO_MSKR = LED_B;
   PIO_GROUP_LED_B.PIO_CFGR = PIO_CFGR_DIR_OUTPUT | PIO_CFGR_FUNC_GPIO;
   PIO_GROUP_LED_B.PIO_SODR = LED_B;

   //Configure up button
   PIO_GROUP_SW_USER.PIO_MSKR = SW_USER;
   PIO_GROUP_SW_USER.PIO_CFGR = PIO_CFGR_PUEN_ENABLED | PIO_CFGR_DIR_INPUT |
      PIO_CFGR_FUNC_GPIO;

   //Configure LAN9250 RST pin (PA25)
   PIO0_REGS->PIO_GROUP[0].PIO_MSKR = PIO_PA25;
   PIO0_REGS->PIO_GROUP[0].PIO_CFGR = PIO_CFGR_DIR_OUTPUT | PIO_CFGR_FUNC_GPIO;

   //Reset LAN9250 Ethernet controller
   PIO0_REGS->PIO_GROUP[0].PIO_CODR = PIO_PA25;
   sleep(100);
   PIO0_REGS->PIO_GROUP[0].PIO_SODR = PIO_PA25;
   sleep(100);
}
...
void userTask(void *param)
{
   ...
   {
      //SW0 button pressed?
      if(!(PIO_GROUP_SW_USER.PIO_PDSR & SW_USER))
      {
         //SCP client test routine
         scpClientTest();

         //Wait for the SW0 button to be released
         while(!(PIO_GROUP_SW_USER.PIO_PDSR & SW_USER));
      }
...
void ledTask(void *param)
{
   ...
   while(1)
   {
      PIO_GROUP_LED_G.PIO_CODR = LED_G;
      osDelayTask(100);
      PIO_GROUP_LED_G.PIO_SODR = LED_G;
      osDelayTask(900);
   }
...
int_t main(void)
{
   ...
   TRACE_INFO("Target: PIC32CX2051MTG128\r\n");
   TRACE_INFO("\r\n");
