...
#define APP_IF3_MAC_ADDR "00-AB-CD-A5-D3-03"
...
#define APP_IF3_IPV6_LINK_LOCAL_ADDR "fe80::a5d3:3"
#define APP_IF3_IPV6_PREFIX "2001:db8::"
#define APP_IF3_IPV6_PREFIX_LENGTH 64
#define APP_IF3_IPV6_GLOBAL_ADDR "2001:db8::a5d3:3"
#define APP_IF3_IPV6_ROUTER "fe80::1"
#define APP_IF3_IPV6_PRIMARY_DNS "2001:4860:4860::8888"
#define APP_IF3_IPV6_SECONDARY_DNS "2001:4860:4860::8844"

//Fourth Ethernet interface configuration
#define APP_IF4_NAME "eth4"
#define APP_IF4_HOST_NAME "http-server-demo-4"
#define APP_IF4_MAC_ADDR "00-AB-CD-A5-D3-04"

#define APP_IF4_USE_DHCP_CLIENT ENABLED
#define APP_IF4_IPV4_HOST_ADDR "192.168.0.24"
#define APP_IF4_IPV4_SUBNET_MASK "255.255.255.0"
#define APP_IF4_IPV4_DEFAULT_GATEWAY "192.168.0.254"
#define APP_IF4_IPV4_PRIMARY_DNS "8.8.8.8"
#define APP_IF4_IPV4_SECONDARY_DNS "8.8.4.4"

#define APP_IF4_USE_SLAAC ENABLED
#define APP_IF4_IPV6_LINK_LOCAL_ADDR "fe80::a5d3:4"
#define APP_IF4_IPV6_PREFIX "2001:db8::"
#define APP_IF4_IPV6_PREFIX_LENGTH 64
#define APP_IF4_IPV6_GLOBAL_ADDR "2001:db8::a5d3:4"
#define APP_IF4_IPV6_ROUTER "fe80::1"
#define APP_IF4_IPV6_PRIMARY_DNS "2001:4860:4860::8888"
#define APP_IF4_IPV6_SECONDARY_DNS "2001:4860:4860::8844"

//Fifth Ethernet interface configuration
#define APP_IF5_NAME "eth5"
#define APP_IF5_HOST_NAME "http-server-demo-5"
#define APP_IF5_MAC_ADDR "00-AB-CD-A5-D3-05"

#define APP_IF5_USE_DHCP_CLIENT ENABLED
#define APP_IF5_IPV4_HOST_ADDR "192.168.0.25"
#define APP_IF5_IPV4_SUBNET_MASK "255.255.255.0"
#define APP_IF5_IPV4_DEFAULT_GATEWAY "192.168.0.254"
#define APP_IF5_IPV4_PRIMARY_DNS "8.8.8.8"
#define APP_IF5_IPV4_SECONDARY_DNS "8.8.4.4"

#define APP_IF5_USE_SLAAC ENABLED
#define APP_IF5_IPV6_LINK_LOCAL_ADDR "fe80::a5d3:5"
#define APP_IF5_IPV6_PREFIX "2001:db8::"
#define APP_IF5_IPV6_PREFIX_LENGTH 64
#define APP_IF5_IPV6_GLOBAL_ADDR "2001:db8::a5d3:5"
#define APP_IF5_IPV6_ROUTER "fe80::1"
#define APP_IF5_IPV6_PRIMARY_DNS "2001:4860:4860::8888"
#define APP_IF5_IPV6_SECONDARY_DNS "2001:4860:4860::8844"
...
DhcpClientContext dhcpClientContext[5];
SlaacSettings slaacSettings;
SlaacContext slaacContext[5];
...
error_t eth4InterfaceInit(void);
error_t eth5InterfaceInit(void);
...
error_t eth1InterfaceInit(void)
{
   ...
   netSetSwitchPort(interface, KSZ9477_PORT1);
#endif
...
error_t eth2InterfaceInit(void)
{
   ...
   //Enable tail tagging mode
   netSetSwitchPort(interface, KSZ9477_PORT2);
...
/**
 * @brief Third Ethernet interface initialization
 **/
...
error_t eth3InterfaceInit(void)
{
   ...
   //Enable tail tagging mode
   netSetSwitchPort(interface, KSZ9477_PORT3);
   //Attach the virtual interface to the relevant physical interface
   netSetParentInterface(interface, physicalInterface);
...
/**
 * @brief Fourth Ethernet interface initialization
 **/
...
error_t eth4InterfaceInit(void)
{
   ...
   NetInterface *physicalInterface;
   MacAddr macAddr;
#if (APP_IF4_USE_DHCP_CLIENT == DISABLED)
   Ipv4Addr ipv4Addr;
#endif
#if (APP_IF4_USE_SLAAC == DISABLED)
   Ipv6Addr ipv6Addr;
#endif

   //Configure the second Ethernet interface
   interface = &netInterface[3];
   //Point to the physical interface
   physicalInterface = &netInterface[0];

   //Set interface name
   netSetInterfaceName(interface, APP_IF4_NAME);
   //Set host name
   netSetHostname(interface, APP_IF4_HOST_NAME);
   //Set host MAC address
   macStringToAddr(APP_IF4_MAC_ADDR, &macAddr);
   netSetMacAddr(interface, &macAddr);

   //Enable tail tagging mode
   netSetSwitchPort(interface, KSZ9477_PORT4);
   //Attach the virtual interface to the relevant physical interface
   netSetParentInterface(interface, physicalInterface);
...
#if (IPV4_SUPPORT == ENABLED)
#if (APP_IF4_USE_DHCP_CLIENT == ENABLED)
   //Get default settings
   dhcpClientGetDefaultSettings(&dhcpClientSettings);
   //Set the network interface to be configured by DHCP
   dhcpClientSettings.interface = interface;
   ...
   //DHCP client initialization
   error = dhcpClientInit(&dhcpClientContext[3], &dhcpClientSettings);
   //Failed to initialize DHCP client?
   if(error)
   {
      //Debug message
   ...
   //Start DHCP client
   error = dhcpClientStart(&dhcpClientContext[3]);
   //Failed to start DHCP client?
   if(error)
   {
      //Debug message
...
#else
   //Set IPv4 host address
   ipv4StringToAddr(APP_IF4_IPV4_HOST_ADDR, &ipv4Addr);
   ipv4SetHostAddr(interface, ipv4Addr);

   //Set subnet mask
   ipv4StringToAddr(APP_IF4_IPV4_SUBNET_MASK, &ipv4Addr);
   ipv4SetSubnetMask(interface, ipv4Addr);

   //Set default gateway
   ipv4StringToAddr(APP_IF4_IPV4_DEFAULT_GATEWAY, &ipv4Addr);
   ipv4SetDefaultGateway(interface, ipv4Addr);

   //Set primary and secondary DNS servers
   ipv4StringToAddr(APP_IF4_IPV4_PRIMARY_DNS, &ipv4Addr);
   ipv4SetDnsServer(interface, 0, ipv4Addr);
   ipv4StringToAddr(APP_IF4_IPV4_SECONDARY_DNS, &ipv4Addr);
   ipv4SetDnsServer(interface, 1, ipv4Addr);
#endif
#endif

#if (IPV6_SUPPORT == ENABLED)
#if (APP_IF4_USE_SLAAC == ENABLED)
   //Get default settings
   slaacGetDefaultSettings(&slaacSettings);
   //Set the network interface to be configured
   slaacSettings.interface = interface;
   ...
   //SLAAC initialization
   error = slaacInit(&slaacContext[3], &slaacSettings);
   //Failed to initialize SLAAC?
   if(error)
   {
      //Debug message
   ...
   //Start IPv6 address autoconfiguration process
   error = slaacStart(&slaacContext[3]);
   //Failed to start SLAAC process?
   if(error)
   {
      //Debug message
...
#else
   //Set link-local address
   ipv6StringToAddr(APP_IF4_IPV6_LINK_LOCAL_ADDR, &ipv6Addr);
   ipv6SetLinkLocalAddr(interface, &ipv6Addr);
#endif
#endif
...
/**
 * @brief Fifth Ethernet interface initialization
 **/
...
error_t eth5InterfaceInit(void)
{
   ...
   NetInterface *physicalInterface;
   MacAddr macAddr;
#if (APP_IF5_USE_DHCP_CLIENT == DISABLED)
   Ipv4Addr ipv4Addr;
#endif
#if (APP_IF5_USE_SLAAC == DISABLED)
   Ipv6Addr ipv6Addr;
#endif

   //Configure the second Ethernet interface
   interface = &netInterface[4];
   //Point to the physical interface
   physicalInterface = &netInterface[0];

   //Set interface name
   netSetInterfaceName(interface, APP_IF5_NAME);
   //Set host name
   netSetHostname(interface, APP_IF5_HOST_NAME);
   //Set host MAC address
   macStringToAddr(APP_IF5_MAC_ADDR, &macAddr);
   netSetMacAddr(interface, &macAddr);

   //Enable tail tagging mode
   netSetSwitchPort(interface, KSZ9477_PORT5);
   //Attach the virtual interface to the relevant physical interface
   netSetParentInterface(interface, physicalInterface);
...
#if (IPV4_SUPPORT == ENABLED)
#if (APP_IF5_USE_DHCP_CLIENT == ENABLED)
   //Get default settings
   dhcpClientGetDefaultSettings(&dhcpClientSettings);
   //Set the network interface to be configured by DHCP
   dhcpClientSettings.interface = interface;
   ...
   //DHCP client initialization
   error = dhcpClientInit(&dhcpClientContext[4], &dhcpClientSettings);
   //Failed to initialize DHCP client?
   if(error)
   {
      //Debug message
   ...
   //Start DHCP client
   error = dhcpClientStart(&dhcpClientContext[4]);
   //Failed to start DHCP client?
   if(error)
   {
      //Debug message
...
#else
   //Set IPv4 host address
   ipv4StringToAddr(APP_IF5_IPV4_HOST_ADDR, &ipv4Addr);
   ipv4SetHostAddr(interface, ipv4Addr);

   //Set subnet mask
   ipv4StringToAddr(APP_IF5_IPV4_SUBNET_MASK, &ipv4Addr);
   ipv4SetSubnetMask(interface, ipv4Addr);

   //Set default gateway
   ipv4StringToAddr(APP_IF5_IPV4_DEFAULT_GATEWAY, &ipv4Addr);
   ipv4SetDefaultGateway(interface, ipv4Addr);

   //Set primary and secondary DNS servers
   ipv4StringToAddr(APP_IF5_IPV4_PRIMARY_DNS, &ipv4Addr);
   ipv4SetDnsServer(interface, 0, ipv4Addr);
   ipv4StringToAddr(APP_IF5_IPV4_SECONDARY_DNS, &ipv4Addr);
   ipv4SetDnsServer(interface, 1, ipv4Addr);
#endif
#endif

#if (IPV6_SUPPORT == ENABLED)
#if (APP_IF5_USE_SLAAC == ENABLED)
   //Get default settings
   slaacGetDefaultSettings(&slaacSettings);
   //Set the network interface to be configured
   slaacSettings.interface = interface;
   ...
   //SLAAC initialization
   error = slaacInit(&slaacContext[4], &slaacSettings);
   //Failed to initialize SLAAC?
   if(error)
   {
      //Debug message
   ...
   //Start IPv6 address autoconfiguration process
   error = slaacStart(&slaacContext[4]);
   //Failed to start SLAAC process?
   if(error)
   {
      //Debug message
...
#else
   //Set link-local address
   ipv6StringToAddr(APP_IF5_IPV6_LINK_LOCAL_ADDR, &ipv6Addr);
   ipv6SetLinkLocalAddr(interface, &ipv6Addr);
#endif
#endif
   ...
   const char_t *param)
{
   ...
   {
      strcpy(connection->buffer, "EVB-KSZ9477");
   }
