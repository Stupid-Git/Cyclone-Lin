...
void debugInit(uint32_t baudrate)
{
   ...
   //Set baudrate
   R_SCI1->BRR = (SystemCoreClock / 1) / (8 * baudrate) - 1;
   ...
   R_MSTP->MSTPCRB &= ~(R_MSTP_MSTPCRB_MSTPB_Msk << 28);
   //Enable protection
   R_SYSTEM->PRCR = 0xA500;
   ...
   //Configure TXD3 (P4_9)
   R_PFS->PORT[4].PIN[9].PmnPFS = (5 << R_PFS_PORT_PIN_PmnPFS_PSEL_Pos) |
      R_PFS_PORT_PIN_PmnPFS_PMR_Msk | R_PFS_PORT_PIN_PmnPFS_PCR_Msk;

   //Configure RXD3 (P4_8)
   R_PFS->PORT[4].PIN[8].PmnPFS = (5 << R_PFS_PORT_PIN_PmnPFS_PSEL_Pos) |
      R_PFS_PORT_PIN_PmnPFS_PMR_Msk | R_PFS_PORT_PIN_PmnPFS_PCR_Msk;
   ...
   R_SCI3->SMR = 0;
   //Configure prescaler for baud rate generator
   R_SCI3->SEMR = R_SCI0_SEMR_BGDM_Msk | R_SCI0_SEMR_ABCS_Msk;
   //Set baudrate
   R_SCI3->BRR = (SystemCoreClock / 1) / (8 * baudrate) - 1;

   //Enable transmission and reception
   R_SCI3->SCR |= R_SCI0_SCR_TE_Msk | R_SCI0_SCR_RE_Msk;
#endif
}
...
#else
      //Wait for the transmitter to be ready
      while((R_SCI3->SSR & R_SCI0_SSR_TEND_Msk) == 0)
      {
      }
