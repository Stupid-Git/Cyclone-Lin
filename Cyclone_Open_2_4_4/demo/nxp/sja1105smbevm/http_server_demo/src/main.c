...
#include "sja1105_config1.h"
#include "sja1105_config2.h"
#include "debug.h"
...
error_t sja1105InitPri(NetInterface *interface);
error_t sja1105InitSec(NetInterface *interface);
...
{
   //Configure LEDs
   SIUL2->MSCR[LED_GP1] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;
   SIUL2->MSCR[LED_GP2] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;
   SIUL2->MSCR[LED_GP3] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;
   SIUL2->MSCR[LED_GP4] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;
   SIUL2->MSCR[LED_GP5] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;
   SIUL2->MSCR[LED_GP6] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;
   SIUL2->MSCR[LED_ALIVE] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;

   //Clear LEDs
   SIUL2->GPDO[LED_GP1] = 1;
   SIUL2->GPDO[LED_GP2] = 1;
   SIUL2->GPDO[LED_GP3] = 1;
   SIUL2->GPDO[LED_GP4] = 1;
   SIUL2->GPDO[LED_GP5] = 1;
   SIUL2->GPDO[LED_GP6] = 1;
   SIUL2->GPDO[LED_ALIVE] = 1;

   //Configure push buttons
   SIUL2->MSCR[PB_SW3] = SIUL2_MSCR_SMC_MASK | SIUL2_MSCR_IBE_MASK;
   SIUL2->MSCR[PB_SW4] = SIUL2_MSCR_SMC_MASK | SIUL2_MSCR_IBE_MASK;
}
...
error_t sja1105InitHook(NetInterface *interface)
{
   ...
   //Configure primary SJA1105 switch
   error = sja1105InitPri(interface);

   //Check status code
   if(!error)
   {
      //Configure secondary SJA1105 switch
      error = sja1105InitSec(interface);
   }
...
/**
 * @brief Configure primary SJA1105 switch
 * @param[in] interface Underlying network interface
 * @return Error code
 **/
...
error_t sja1105InitPri(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Configuring primary switch...\r\n");

   //Configure primary switch reset pin
   SIUL2->MSCR[SJA1105_RST1] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;

   //Reset primary switch
   SIUL2->GPDO[SJA1105_RST1] = 0;
   sleep(10);
   SIUL2->GPDO[SJA1105_RST1] = 1;
   sleep(10);
   ...
   error = sja1105LoadStaticConfig(interface, sja1105Config1,
      osStrlen(sja1105Config1));
   ...
   sja1105ConfigurePll1(interface);
   ...
   //Enable TJA1102 PHY
   SIUL2->MSCR[TJA1102_EN1] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;
   SIUL2->GPDO[TJA1102_EN1] = 1;

   //Configure PHY reset pins
   SIUL2->MSCR[TJA1102_RST1] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;
   SIUL2->MSCR[KSZ9031_RST1] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;
   SIUL2->MSCR[KSZ9031_RST2] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;

   //Reset PHY transceivers
   SIUL2->GPDO[TJA1102_RST1] = 0;
   SIUL2->GPDO[KSZ9031_RST1] = 0;
   SIUL2->GPDO[KSZ9031_RST2] = 0;
   sleep(10);
   SIUL2->GPDO[TJA1102_RST1] = 1;
   SIUL2->GPDO[KSZ9031_RST1] = 1;
   SIUL2->GPDO[KSZ9031_RST2] = 1;
   sleep(10);
...
/**
 * @brief Configure secondary SJA1105 switch
 * @param[in] interface Underlying network interface
 * @return Error code
 **/
...
error_t sja1105InitSec(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Configuring secondary switch...\r\n");

   //Configure secondary switch reset pin
   SIUL2->MSCR[SJA1105_RST2] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;

   //Reset secondary switch
   SIUL2->GPDO[SJA1105_RST2] = 0;
   sleep(10);
   SIUL2->GPDO[SJA1105_RST2] = 1;
   sleep(10);
   ...
   //Load the static configuration into the device
   error = sja1105LoadStaticConfig(interface, sja1105Config2,
      osStrlen(sja1105Config2));
   ...
   //Enable TJA1102 PHYs
   SIUL2->MSCR[TJA1102_EN2] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;
   SIUL2->MSCR[TJA1102_EN3] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;
   SIUL2->GPDO[TJA1102_EN2] = 1;
   SIUL2->GPDO[TJA1102_EN3] = 1;

   //Configure PHY reset pin
   SIUL2->MSCR[TJA1102_RST2] = SIUL2_MSCR_OBE_MASK | SIUL2_MSCR_SMC_MASK;

   //Reset PHY transceivers
   SIUL2->GPDO[TJA1102_RST2] = 0;
   sleep(10);
   SIUL2->GPDO[TJA1102_RST2] = 1;
   sleep(10);

   //Loop through the PHYs attached to the secondary switch
   for(interface->phyAddr = 6; interface->phyAddr <= 9; interface->phyAddr++)
   {
      //Configure the PHY
      error = tja1102Init(interface);
      //Any error to report?
...
bool_t sja1105GetLinkState(NetInterface *interface, uint8_t port)
{
   ...
   bool_t linkState;
   uint32_t linkSpeed;
   static bool_t prevLinkState[2] = {FALSE, FALSE};

   //Check port number
   if(port == SJA1105_PORT1)
   {
      //Read status register
      value = sja1105ReadPhyReg(interface, 4, TJA1102_BASIC_STAT);
   ...
   {
      //Read status register
      value = sja1105ReadPhyReg(interface, 2, KSZ9031_BMSR);
      //Retrieve current link state
      linkState = (value & KSZ9031_BMSR_LINK_STATUS) ? TRUE : FALSE;

      //Link up event?
      if(linkState && !prevLinkState[0])
      {
         //Get current link speed
         linkSpeed = sja1105GetLinkSpeed(interface, SJA1105_PORT2);
         //Reconfigure the port speed through runtime interface
         sja1105SetPortSpeed(interface, SJA1105_PORT2, linkSpeed);
         //Adjust CGU parameters
         sja1105ConfigureCgu(interface, SJA1105_PORT2);
      }

      //Save current link state
      prevLinkState[0] = linkState;
   }
   else if(port == SJA1105_PORT3)
   {
      //Read status register
      value = sja1105ReadPhyReg(interface, 3, KSZ9031_BMSR);
      //Retrieve current link state
      linkState = (value & KSZ9031_BMSR_LINK_STATUS) ? TRUE : FALSE;

      //Link up event?
      if(linkState && !prevLinkState[1])
      {
         //Get current link speed
         linkSpeed = sja1105GetLinkSpeed(interface, SJA1105_PORT3);
         //Reconfigure the port speed through runtime interface
         sja1105SetPortSpeed(interface, SJA1105_PORT3, linkSpeed);
         //Adjust CGU parameters
         sja1105ConfigureCgu(interface, SJA1105_PORT3);
      }

      //Save current link state
      prevLinkState[1] = linkState;
   }
   else if(port == SJA1105_PORT4)
   {
      ...
      //Loop through the PHYs attached to the secondary switch
      for(phyAddr = 6; phyAddr <= 9; phyAddr++)
      {
         //Read status register
         value = sja1105ReadPhyReg(interface, phyAddr, TJA1102_BASIC_STAT);
...
uint32_t sja1105GetLinkSpeed(NetInterface *interface, uint8_t port)
{
   ...
   //Check port number
   if(port == SJA1105_PORT0 && port == SJA1105_PORT1)
   {
      linkSpeed = NIC_LINK_SPEED_100MBPS;
   }
   else if(port == SJA1105_PORT2 || port == SJA1105_PORT3)
   {
      //Get the address of the PHY
      phyAddr = (port == SJA1105_PORT2) ? 2 : 3;

      //Read PHY control register
      value = sja1105ReadPhyReg(interface, phyAddr, KSZ9031_PHYCON);
   ...
   const char_t *param)
{
   ...
   {
      strcpy(connection->buffer, "SJA1105SMBEVM");
   }
