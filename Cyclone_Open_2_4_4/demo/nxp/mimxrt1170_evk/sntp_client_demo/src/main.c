...
#define APP_IF1_HOST_NAME "sntp-client-demo-1"
...
#define APP_IF2_HOST_NAME "sntp-client-demo-2"
...
error_t sntpClientTest(NetInterface *interface)
{
   error_t error;
   uint32_t kissCode;
   time_t unixTime;
   ...
   do
   {
      ...
      error = getHostByName(interface, APP_NTP_SERVER_NAME, &ipAddr, 0);
      //Any error to report?
      if(error)
      {
         //Debug message
      ...
      //Bind the SNTP client to the relevant network interface
      error = sntpClientBindToInterface(&sntpClientContext, interface);
      //Any error to report?
      if(error)
         break;
   ...
   while(1)
   {
      ...
      if(!GPIO_PinRead(BOARD_USER_BUTTON_GPIO, BOARD_USER_BUTTON_GPIO_PIN))
      {
         if(netGetLinkState(&netInterface[0]))
         {
            ...
            sntpClientTest(&netInterface[0]);
         }
         else if(netGetLinkState(&netInterface[1]))
         {
            //SNTP client test routine
            sntpClientTest(&netInterface[1]);
         }
