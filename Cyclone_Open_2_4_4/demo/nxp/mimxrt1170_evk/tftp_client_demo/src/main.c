...
#define APP_IF1_HOST_NAME "tftp-client-demo-1"
...
#define APP_IF2_HOST_NAME "tftp-client-demo-2"
...
error_t tftpClientTest(NetInterface *interface)
{
   error_t error;
   uint_t i;
   size_t n;
   ...
   do
   {
      ...
      error = getHostByName(interface, APP_TFTP_SERVER_NAME, &ipAddr, 0);
      //Any error to report?
      if(error)
      {
         //Debug message
      ...
      //Bind the TFTP client to the relevant network interface
      error = tftpClientBindToInterface(&tftpClientContext, interface);
      //Any error to report?
      if(error)
         break;
   ...
   while(1)
   {
      ...
      if(!GPIO_PinRead(BOARD_USER_BUTTON_GPIO, BOARD_USER_BUTTON_GPIO_PIN))
      {
         if(netGetLinkState(&netInterface[0]))
         {
            ...
            tftpClientTest(&netInterface[0]);
         }
         else if(netGetLinkState(&netInterface[1]))
         {
            //TFTP client test routine
            tftpClientTest(&netInterface[1]);
         }
