...
#define APP_MAC_ADDR "00-AB-CD-EF-58-24"
...
#define APP_IPV6_LINK_LOCAL_ADDR "fe80::5824"
#define APP_IPV6_PREFIX "2001:db8::"
#define APP_IPV6_PREFIX_LENGTH 64
#define APP_IPV6_GLOBAL_ADDR "2001:db8::5824"
...
void adcInit(void)
{
   ...
   hsadc_converter_config_t hsadcConverterConfig;
   hsadc_sample_config_t hsadcSampleConfig;
   ...
   HSADC_Init(HSADC0, &hsadcConfig);

   //Configure converter
   HSADC_GetDefaultConverterConfig(&hsadcConverterConfig);
   HSADC_SetConverterConfig(HSADC0, kHSADC_ConverterA, &hsadcConverterConfig);

   //Power up converter
   HSADC_EnableConverterPower(HSADC0, kHSADC_ConverterA, true);

   //Poll the status bits until the converter is powered up
   while((HSADC_GetStatusFlags(HSADC0) & kHSADC_ConverterAPowerDownFlag) != 0)
   {
   }
   ...
   //Configure the sample slot
   hsadcSampleConfig.channelNumber = 6;
   hsadcSampleConfig.channel67MuxNumber = 3;
   hsadcSampleConfig.enableDifferentialPair = false;
   HSADC_SetSampleConfig(HSADC0, 0, &hsadcSampleConfig);

   //Enable the sample slot
   sampleMask = HSADC_SAMPLE_MASK(0);
   HSADC_EnableSample(HSADC0, sampleMask, true);
   HSADC_EnableSample(HSADC0, ~sampleMask, false);

   //Perform hardware calibration
   HSADC_DoAutoCalibration(HSADC0, kHSADC_ConverterA,
      kHSADC_CalibrationModeSingleEnded | kHSADC_CalibrationModeDifferential);

   //Wait the calibration process complete
   while((HSADC_GetStatusFlags(HSADC0) &
...
uint16_t adcGetValue(void)
{
   ...
   while((HSADC_GetStatusFlags(HSADC0) & kHSADC_ConverterAEndOfScanFlag) == 0)
   {
   }

   //Get A/D conversion result
   value = HSADC_GetSampleResultValue(HSADC0, 0);
