...
void ioInit(void)
{
   ...
   GPIO_PinInit(GPIO, LED0_PORT, LED0_PIN, &pinConfig);
...
void userTask(void *param)
{
   ...
   {
      //SW2 button pressed?
      if(!GPIO_PinRead(GPIO, SW2_PORT, SW2_PIN))
      {
         //FTP client test routine
         ftpClientTest();

         //Wait for the SW2 button to be released
         while(!GPIO_PinRead(GPIO, SW2_PORT, SW2_PIN));
      }
...
void ledTask(void *param)
{
   ...
   while(1)
   {
      GPIO_PinWrite(GPIO, LED0_PORT, LED0_PIN, 0);
      osDelayTask(100);
      GPIO_PinWrite(GPIO, LED0_PORT, LED0_PIN, 1);
      osDelayTask(900);
   }
}
