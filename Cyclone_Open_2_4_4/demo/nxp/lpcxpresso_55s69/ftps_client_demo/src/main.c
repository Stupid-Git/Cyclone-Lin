...
void ioInit(void)
{
   ...
   GPIO_PinInit(GPIO, BOARD_LED_RED_GPIO_PORT, BOARD_LED_RED_GPIO_PIN,
      &pinConfig);

   GPIO_PinInit(GPIO, BOARD_LED_GREEN_GPIO_PORT, BOARD_LED_GREEN_GPIO_PIN,
      &pinConfig);

   GPIO_PinInit(GPIO, BOARD_LED_BLUE_GPIO_PORT, BOARD_LED_BLUE_GPIO_PIN,
      &pinConfig);

   //Enable weak pull-up on SW1, SW2 and SW3
   IOCON_PinMuxSet(IOCON, BOARD_SW1_GPIO_PORT, BOARD_SW1_GPIO_PIN,
      IOCON_FUNC0 | IOCON_MODE_PULLUP | IOCON_DIGITAL_EN | IOCON_INPFILT_OFF);

   IOCON_PinMuxSet(IOCON, BOARD_SW2_GPIO_PORT, BOARD_SW2_GPIO_PIN,
      IOCON_FUNC0 | IOCON_MODE_PULLUP | IOCON_DIGITAL_EN | IOCON_INPFILT_OFF);

   IOCON_PinMuxSet(IOCON, BOARD_SW3_GPIO_PORT, BOARD_SW3_GPIO_PIN,
      IOCON_FUNC0 | IOCON_MODE_PULLUP | IOCON_DIGITAL_EN | IOCON_INPFILT_OFF);

   //Configure SW1, SW2 and SW3 as inputs
   pinConfig.pinDirection = kGPIO_DigitalInput;
   GPIO_PinInit(GPIO, BOARD_SW1_GPIO_PORT, BOARD_SW1_GPIO_PIN, &pinConfig);
   GPIO_PinInit(GPIO, BOARD_SW2_GPIO_PORT, BOARD_SW2_GPIO_PIN, &pinConfig);
   GPIO_PinInit(GPIO, BOARD_SW3_GPIO_PORT, BOARD_SW3_GPIO_PIN, &pinConfig);
}
...
void userTask(void *param)
{
   ...
   while(1)
   {
      ...
      if(!GPIO_PinRead(GPIO, BOARD_SW1_GPIO_PORT, BOARD_SW1_GPIO_PIN))
      {
         ...
         while(!GPIO_PinRead(GPIO, BOARD_SW1_GPIO_PORT, BOARD_SW1_GPIO_PIN));
      }
...
int_t main(void)
{
   ...
   //Set VBAT brown out detect level
