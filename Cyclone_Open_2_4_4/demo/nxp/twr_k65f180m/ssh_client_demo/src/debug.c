...
void debugInit(uint32_t baudrate)
{
   ...
   PORT_SetPinConfig(PORTE, 16, &portPinConfig);

   //Configure UART2_RX (PTE17)
   ...
   portPinConfig.mux = kPORT_MuxAlt3;
   portPinConfig.lockRegister = kPORT_UnlockRegister;
   PORT_SetPinConfig(PORTE, 17, &portPinConfig);

   //Get UART module clock frequency
   freq = CLOCK_GetCoreSysClkFreq() / 3;
   ...
   UART_Init(UART2, &uartConfig, freq);
