   ...
   size_t oidLen, const MibVariant *value, size_t valueLen, bool_t commit)
{
   ...
   if(!strcmp(object->name, "ledState"))
   {
      ...
      if(commit)
      {
         ...
         {
            if(entry->ledState)
               GPIO_PinWrite(BOARD_LED_YELLOW_GPIO, BOARD_LED_YELLOW_GPIO_PIN, 0);
            else
               GPIO_PinWrite(BOARD_LED_YELLOW_GPIO, BOARD_LED_YELLOW_GPIO_PIN, 1);
         }
         else if(index == 3)
         {
            if(entry->ledState)
               GPIO_PinWrite(BOARD_LED_ORANGE_GPIO, BOARD_LED_ORANGE_GPIO_PIN, 0);
            else
               GPIO_PinWrite(BOARD_LED_ORANGE_GPIO, BOARD_LED_ORANGE_GPIO_PIN, 1);
         }
      }
   }
   //Unknown object?
